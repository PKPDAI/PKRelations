...
import argparse
from pkrex.utils import read_jsonl, write_jsonl, make_super_tagger, get_link, resolve_overlapping_spans, \
...
def run(path_inp_file: str, path_out_file: str, path_base_model: str, path_ner_dict: str, only_link: bool):
    inp_data = list(read_jsonl(path_inp_file))
    if only_link:
        print("Only appending link")
        out_sentences = []
        for sentence in tqdm(inp_data):
            sentence['meta'] = {"url": get_link(sentence)}
            out_sentences.append(sentence)
    else:
        print("Applying NER model")
        ...
        for n, (sentence, spacy_doc) in tqdm(enumerate(zip(inp_data, spacy_model.pipe(raw_text, n_process=12)))):
            ...
            sentence['spans'] = resolve_overlapping_spans(spans)
            sentence['meta'] = {"url": get_link(sentence)}
